...
import net.minecraft.client.gui.DrawContext; // Новый импорт для 1.20
...
public class MoveHudScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        // В 1.20 фон экрана рисуется через контекст
        this.renderBackground(context);
        MinecraftClient client = MinecraftClient.getInstance();
        TextRenderer renderer = this.textRenderer;

        // Передаем контекст для жёлтого заголовка
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // В 1.20 работа с матрицами идет через context.getMatrices()
            ...
            // Вывод текста через новый метод context.drawTextWithShadow
    ...
    private void drawScaledTexturePanel(DrawContext context, int x1, int y1, int x2, int y2) {
        ...
        // ИСПРАВЛЕНО: Рендеринг текстур полностью переведён на сигнатуру context.drawTexture из 1.20
        // Левая часть
    ...
    private void drawCenteredString(DrawContext context, TextRenderer renderer, String s, int x, int y, int color) {
        // ИСПРАВЛЕНО: Центрированный текст теперь тоже рисуется через context.drawTextWithShadow
