...
import net.minecraft.client.gui.DrawContext; // ИСПРАВЛЕНО: Новый импорт вместо DrawableHelper
...
public class HudRenderer {
    ...
    public static void register() {
        // ИСПРАВЛЕНО: В 1.20 метод HudRenderCallback принимает drawContext вместо matrixStack
        HudRenderCallback.EVENT.register((drawContext, tickDelta) -> {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                // Получаем доступ к матрице внутри нового контекста отрисовки 1.20
                ...
                // Передаем новый drawContext в метод отрисовки фона
                drawTexturePanel(drawContext, x1, y1, x2, y2);

                // Отрисовка текста поверх текстуры через drawContext в 1.20
                if (i == 1) {
                    drawContext.drawTextWithShadow(renderer, "FPS: " + HudData.lastFps + " ", 0, 0, baseColor);
    ...
    private static void drawTexturePanel(DrawContext context, int x1, int y1, int x2, int y2) {
        // ИСПРАВЛЕНО: В 1.20 шейдеры и текстуры биндятся автоматически внутри методов context.drawTexture
